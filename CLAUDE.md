...
```
Read: memoria-next/error_other.log
```

過去のインシデントが記録されている。読まずに作業を始めると同じ失敗を繰り返す。
**サーバー作業はSSHコマンドをユーザーに渡して実行してもらう。自分でSSH接続しない。**
- コマンドを提案する前に「この出力は今の問題に直接役立つか？」を自問する。NOなら出さない。
- pm2再起動後の動作確認はブラウザテストで行う。再起動後のログは古い内容が残るだけ。
